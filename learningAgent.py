...
class ReinforceAlgorithm():
    ...
    def  solver(self):
        for episode in range(self.numberEpisodes):
            ...
            state=np.random.randint(200-(self.numStates)/2,200+(self.numStates)/2+1)
            monprice=(state + self.env.costs[0]) / 2
            action=np.random.randint(monprice-self.numActions, monprice+1)
            advAction=self.chooseAdver(state)

            reward=(state-action)*(action-self.env.costs[0])
            next_state=state+.5*(advAction-action)
            opt_value_next=max(self.Qtable[next_state-200+self.numStates/2])
           #CHECK INDICES!
            #WRITE ALPHA FUNCTION
            qvalue=self.Qtable[state,action](reward+self.gamma*opt_value_next)
