    ...
    def solver(self, print_step=10_000, prob_break_limit_ln=-0.001,clc=0.1):
        ...
        for iteration in range(self.numberIterations):
            ...
            for episode in range(self.numberEpisodes):
                ...
                discReturns = (self.returnsComputation(rewards, episodeMemory))/10_000



                actor_loss = - (torch.sum((discReturns-(torch.stack(value_lst)).detach())*action_logprobs)) / \
                    len(episodeMemory)
                critic_loss= torch.sum(torch.pow(torch.stack(value_lst)-discReturns,2))
                ...
                if (episode % print_step == 0) or shouldBreak:
                    ...
                    print("loss= ", loss, "  , actor= ", actor_loss,"  , critic= ", critic_loss, "  , return= ", returns)
                    # print("states= ", states)
