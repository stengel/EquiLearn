...
class DemandPotentialGame():
    ...
    def resetGame(self):
        ...
        self.demandPotential = [
            [0]*(self.T), [0]*(self.T)]  # two lists for the two players
    ...
    def guess(self, player, firstprice):  # predictive fighting strategy
        ...
        prevsales = self.demandPotential[1 -
                                         player][t-1] - self.prices[1-player][t-1]
...
class Model(DemandPotentialGame):
    ...
    def __init__(self, totalDemand, tupleCosts, totalStages, adversaryProbs, advHistoryNum=3) -> None:
        ...
        self.initState = [
            0, totalDemand/2, ((totalDemand/2) + tupleCosts[0])/2] + ([0]*advHistoryNum)
    ...
    def step(self, state, action):
        ...
        Parameters:
        - action: Price
        - state: tupple in the latest stage (stage ,Demand Potential, Agent's Price, Adversary's price hisotry)
        """
        adversaryAction = self.adversaryChoosePrice()
        myopicPrice = self.myopic()
        self.updatePricesProfitDemand(
            [self.myopic() - action, adversaryAction])

        done = (self.stage == self.T-1)

        
        if not done:
            newState = [self.stage+1, self.demandPotential[0]
                        [self.stage + 1], self.prices[0][self.stage]] 
        else:
            newState = [self.stage+1, 0, self.prices[0]
                        [self.stage]] 

        if(self.advHistoryNum>0):
