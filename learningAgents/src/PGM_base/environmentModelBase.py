...
class Model(DemandPotentialGame):
    ...
    def getState(self,stage):
        ...
        if stage==0:
            state= stageEncode+[self.totalDemand/2, ((self.totalDemand/2) + self.costs[0])/2] + ([0]*self.advHistoryNum)

        else:
            #check last stageeee demand
            state = stageEncode+[self.demandPotential[0][stage], self.prices[0][stage-1]] 
