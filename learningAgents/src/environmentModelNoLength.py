...
# Without a fixed length of the game
...
class DemandPotentialGame():
    ...
    def __init__(self, totalDemand, tupleCosts, terminalProb) -> None:
        ...
        self.prices = None # prices over the rounds
    ...
    def resetGame(self):
        ...
        """
        self.demandPotential = [list(),list()] # two lists for the two players
        self.prices = [list(),list()]  # prices over the rounds
        self.profit = [list(),list()]  # profit in each round
        self.demandPotential[0].append(self.totalDemand/2) # initialise round 0
        self.demandPotential[1].append(self.totalDemand/2)
    ...
    def updatePricesProfitDemand(self, pricePair):
        ...
        for player in [0,1]:
            ...
            self.prices[player].append(price)
            profit = (self.demandPotential[player][self.stage] - price)*(price - self.costs[player])
            self.profit[player].append(profit)
            demandPotential = self.demandPotential[player][self.stage] + (pricePair[1-player] - price)/2
            self.demandPotential[player].append(demandPotential)
...
class Model(DemandPotentialGame):
    ...
    """
    def __init__(self, totalDemand,tupleCosts,terminalProb, initState,adversaryMode) -> None:
        super().__init__( totalDemand,tupleCosts,terminalProb)
    ...
    def step(self, state, action):
        ...
        # This is only of the opponent, which is fine for the demand potential, but maybe not the prices? 
        reward = self.rewardFunction()
        self.stage = self.stage + 1

        terminalDist = bernoulli(self.p)
        done = terminalDist.rvs(1)
        return torch.tensor(newState, dtype=torch.float32), reward, done == 1
