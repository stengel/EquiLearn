...
if __name__ == "__main__":
    ...
    bimatrix_game = cl.BimatrixGame(
        low_cost_strategies=[strt1, strt2, strt3], high_cost_strategies=[strt1, strt2, strt3,strtH1,strtH2], env_class=env_class)
    ...
    for round in range(num_rounds):
        ...
        [acceptable, agent_payoffs, adv_payoffs, agent_strategy, expected_payoff] = training(env_class=env_class, costs=[
                                                                                             gl.LOW_COST, gl.HIGH_COST], adv_mixed_strategy=\
                                                                                                high_mixed_strat, target_payoff=\
                                                                                                    equi["low_cost_payoff"],num_procs=num_procs)
        if acceptable:
            new_low=True
            # update[int(i/2)] = True
        ...
        [acceptable, agent_payoffs, adv_payoffs, agent_strategy, expected_payoff] = training(env_class=env_class, costs=[
                                                                                            ...
                                                                                            low_mixed_strat, target_payoff=\
                                                                                                equi["high_cost_payoff"],num_procs=num_procs)
        if acceptable:
            new_high=True
