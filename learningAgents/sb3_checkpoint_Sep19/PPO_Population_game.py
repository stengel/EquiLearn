...
def training(env_class, costs, adv_mixed_strategy, target_payoff):
    """
    ...
    train_env = make_vec_env(env_class, n_envs=gl.NUM_PROCESS, seed=seed, vec_env_cls=SubprocVecEnv, env_kwargs=dict(
        tuple_costs=costs, adversary_mixed_strategy=adv_mixed_strategy))
    ...
    for strategy_index in range(len(adv_mixed_strategy.strategies)):
        if adv_mixed_strategy.strategy_probs[strategy_index] > 0:
            ...
            for i in range(gl.NUM_STOCHASTIC_ITER):
                ...
                sum(pricing_game.profit[0]), sum(pricing_game.profit[1]),  str(pricing_game.profit[0]), str(pricing_game.actions), str(pricing_game.prices[0]),\
                str(pricing_game.prices[1]), str(pricing_game.demand_potential[0]),str(pricing_game.demand_potential[1]), gl.LR, gl.NUM_ADV_HISTORY,\
                gl.TOTAL_STAGES, gl.ACTION_STEP, gl.NUM_ACTIONS, gl.GAMMA, False, seed, gl.NUM_PROCESS, running_time]
    ...
    data = [model_name, number_episodes, ("L" if (costs[0] < costs[1]) else "H"), str(adv_mixed_strategy), expected_payoff, target_payoff,
            gl.LR, gl.NUM_ADV_HISTORY, gl.TOTAL_STAGES, gl.ACTION_STEP, gl.NUM_ACTIONS, gl.GAMMA, seed, gl.NUM_PROCESS, running_time,
...
if __name__ == "__main__":
    ...
    for round in range(num_rounds):
        ...
        [acceptable, agent_payoffs, adv_payoffs, agent_strategy, expected_payoff] = training(env_class=env_class, costs=[
                                                                                             gl.LOW_COST, gl.HIGH_COST], adv_mixed_strategy=\
                                                                                                high_mixed_strat, target_payoff=\
                                                                                                    equi["low_cost_payoff"])
        if acceptable:
            new_low=True
            # update[int(i/2)] = True
        ...
        [acceptable, agent_payoffs, adv_payoffs, agent_strategy, expected_payoff] = training(env_class=env_class, costs=[
                                                                                            gl.HIGH_COST, gl.LOW_COST], adv_mixed_strategy=\
                                                                                            low_mixed_strat, target_payoff=\
                                                                                                equi["high_cost_payoff"])
        if acceptable:
            new_high=True
