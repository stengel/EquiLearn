...
% \newcommand{\REF}[2]{{\scriptsize\B{\href{#1}{[#2]}}}}
...
\date{June 2023
% \\ \REF{http://www.maths.lse.ac.uk/Personal/stengel/}{Click on references for URLs}
}
...
% \0 existing \5{human-designed} strategies for strategic tournament
...
\8
new strategy added to a \B{population game}
...
Total demand potential $400$ split as $\R{D_1}+\B{D\us2}$
between two producing firms with costs $\R{c_1=57}$ and
$\B{c_2=71}$.

Firm $i$ \5{chooses \7{price} $p_i$} and \7{sells}
$D_i-\5{p_i}$
units, gets \7{{profit}} $(D_i-\5{p_i})(\5{p_i}-c_i)$\pause
${}=-\5{p_i\ttp2}+(D_i+c_i)\5{p_i}-D_ic_i$
...
\]
Total profits over 25 stages about \R{$156$k}, \B{$109$k}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\begin{frame}
{Strategy experiments \B{[Keser 1993]}}
...
\pause
\0
\R{Used now to \7{custom-design and optimize an agent}}
...
\0
\R{Base game} = pricing game over 25 stages, in two roles
...
\pause
\0
\B{strategy}  represented by an \B{RL agent}
that chooses the \7{next price} as a function of
data for the last, say, $3$ stages
...
\strut\hfill
\includegraphics[width=.7\hsize]{rl.jpg} 
\hfill\strut
\vpause
\0
here: \R{(mixed) equilibrium} as learning environment,
\R{random} but \B{constant} (not evolving with the
learning agent)
\vpause
\0 tested RL methods so far:
\8
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\begin{frame}
{Learning a new strategy: \fbox{not easy}}
...
\0
assumption: learn next price as \7{function} of last $3$
stages with
\8
information per state : \R{own price, own profit}, opponent price
\8
explicit \R{\7{state}: demand potential, stage}

\8 e.g. using
Policy Gradient Method (PGM):
\includegraphics[width=.9\hsize]{policystate.png} 

\0
learning is \7{slow} -- Q-learning needs large Q-table
...
\8 aim for a ``fair split'' of \B{demand potential}

\8 \7{predict} \R{opponent price} with multiplicative weights

\8 set own price to achieve \7{target} demand potential

\8 use \R{somewhat lower} price to steal customers

\pause
\0
\B{optimization} of ${}\sim4$ parameters close to human-designed
agent
(\B{not by gradient descent}, too many ``cliffs'')
...
\8
defines \7{bimatrix game} with \R{new equilibrium} as next
learning environment 

\includegraphics[width=1\hsize]{mixedlearn.png} 
...
random starting pair as \7{prior} (Harsanyi-Selten tracing procedure)
\8 as \7{\R{(accurate??)} proxy for \B{evolutionary dynamics}} 
\8 finds only positive-\7{index} equilibria (for dynamic
stability)
...
$(\R B,\B b)$ = \7{PGM-learned} pure NE, stops at size $8\times 5$ (SPNE?)

\pause
Restricted to equilibrium supports:

\[
\bimatrixgame{1.45em}{2}{3}{1}{2}%
{{$A$}{$B$}}%
{{myopic}{$a$}{$b$}}
{
...
\put(-3,-2){\mm{$0.875$}}
\put(-3,-6){\mm{$0.125$}}
\put(13.4,-3.5){\mm{\R{$148.76$}}}
\put(15,-1.5){\mm{\B{$75.75$}}}
% \payoffpairs{1}{{}{}{}}{{}{}{}{$75.75$}}
}
\]
...
\8 potentially has ``hand-made'' good (benchmark?) strategies
...
\0
RL agents learn \7{slowly} (investigate further approaches)
\8
often too jittery (oscillations)

\0
Learning for \7{evolutionary success} different from
high-payoff
